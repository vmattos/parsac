...
public class CursoTest {
	...
	public void testaCurso() {
		...
		for (Secao secao : curso.getSecoes()) {
			...
			Assert.assertEquals("View", exercicioMultiplaEscolha
					.getAlternativas().get(1).getTexto());
			Assert.assertEquals("Model", exercicioMultiplaEscolha
					.getAlternativas().get(2).getTexto());
			Assert.assertEquals("Espalhada em todas as camadas do MVC",
					exercicioMultiplaEscolha.getAlternativas().get(3)
							.getTexto());
